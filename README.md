...
| 💬 Упоминание `@bot` | RAG-ответ на вопрос по всей истории чата |
...
> ⚠️ **Критически важно:** без отключения Privacy Mode бот **не будет получать обычные сообщения** в группе и не сможет отвечать на `@упоминания`.
...
3. После добавления в группу бот автоматически начнёт индексировать все сообщения
